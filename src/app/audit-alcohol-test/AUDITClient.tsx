...
export function AUDITClient({ faqData, hideTherapyCTA = false }: Props) {
  ...
  const handleShare = useCallback(async (mode: "share" | "copy") => {
    const outcome = await sharePrivateToolLink({
      toolName: "AUDIT Alcohol Use Screen",
      canonicalPath: "/audit-alcohol-test",
      mode,
    });
    if (outcome === "copied") {
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
                ...
                >
                  <div className="flex gap-3 items-start">
                    ...
                    <div className="flex-1 min-w-0">
                      <p id={`audit-q-${qi}`} className="text-[15px] font-medium text-neutral-800 dark:text-neutral-100 leading-relaxed mb-3">{q.text}</p>
                      <div role="radiogroup" aria-labelledby={`audit-q-${qi}`} className={`grid gap-2 ${q.options.length <= 3 ? "grid-cols-1 sm:grid-cols-3" : "grid-cols-2 sm:grid-cols-3"}`}>
                        {q.options.map((opt) => (
                          <button
                            key={opt.label}
                            role="radio"
      ...
      {showResults && (
        <div ref={resultsRef} className="animate-fade-in" aria-live="polite">
          ...
          <ReflectionSummary
            toolName="AUDIT Alcohol Use Screen"
            toolUrl="https://mindchecktools.com/audit-alcohol-test"
            score={totalScore}
          ...
          {/* Reflection Prompts */}
          {REFLECTION_PROMPTS["audit-alcohol-test"] && (
            <ReflectionPrompts
              toolName="AUDIT Alcohol Use Screen"
          ...
          <footer className="border-t border-sand-200 dark:border-neutral-700 pt-6 text-center">
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400">This tool is for educational purposes only. It is not medical advice, a diagnosis, or a treatment recommendation.</p>
            <ToolReviewerBio />
          </footer>
        </div>
      )}
