...
export function GAD7Client({ faqData, suppressTherapyCTA = false }: Props) {
  ...
  const handleShare = useCallback(async (mode: "share" | "copy") => {
    const outcome = await sharePrivateToolLink({
      ...
      canonicalPath: "/gad-7-anxiety-test",
      mode,
    });
    if (outcome === "copied") {
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      <header className="mb-8">
        ...
        <p className="text-neutral-500 dark:text-neutral-400 leading-relaxed max-w-xl">
          A validated screening questionnaire that helps you reflect on anxiety symptoms over the past two weeks. Questionnaire answers and scores are processed locally and are not intentionally sent to MindCheck Tools.
        </p>
