...
## September 5, 2026 source correction and alcohol/drug-only editorial scope
...
The owner authorized verifying and proceeding with the page-3 correction, and explicitly restricted new articles to alcohol and drugs until an LCSW joins the team. This is authorization for a source/date correction, not a claim that the owner performed a new clinical review. Preserve the original August 26 review and publication dates; show September 5 separately as the source/date correction and actual modification date.

The former SAMHSA week-toolkit URL redirects to the generic Digital Toolkits directory. However, a fresh disconfirming search found [SAMHSA's original August 31, 2026 announcement](https://www.samhsa.gov/blog/convening-highlights-samhsa-commitment-help-states-sud-treatment-recovery-support-services), which explicitly identifies August 25 through August 31, 2026 as Overdose Awareness Week. The September 5 report's earlier proposal saying the range was unverified is superseded. Direct HTTPS retrieval returned HTTP 200 and the date statement; the research browser returned 403, so an access failure was not treated as absence of evidence. [CDC's current toolkit](https://www.cdc.gov/overdose-prevention/php/toolkits/ioad.html) and [the campaign organizer](https://www.overdoseday.com/) independently confirm August 31 as the international day. [End Overdose](https://endoverdose.net/campaign/) confirms only its own August 2026 month campaign, not a universal federal designation.
...
The matching FAQ now attributes the same year-specific range to that announcement, retains August 31 as the international day, and allows a separate local event date. Only the month/week paragraph, affected FAQ, source destination, correction metadata, and corresponding tests change. The fentanyl article object, images, other overdose content, safety instructions, and original reviewer attribution remain unchanged. The exact-content hash is intentionally updated for this authorized source-only correction; it is not evidence of a newly performed clinical review. Source and credential uncertainty elsewhere is not cleared by this change.

New-article boundary: draft and publish only alcohol/drug education within relevant expertise. General mental-health and mental-load article expansion is deferred until an LCSW joins the team, their active credential and relevant expertise are verified, the owner reopens that lane, and the exact content receives documented review. Do not treat an LCSW or CADC-II credential as automatic authority for medication or detox protocols. Existing mental-health tools remain available and receive necessary safety maintenance. The mixed-topic hub, wellness, and grief drafts remain withheld. A possible next article is a practical alcohol/drug recovery trigger-worksheet companion, not a general mental-health article; it remains a topic hypothesis requiring source work and exact-content review.
...
1. Apply the alcohol/drug-only new-article scope in AGENTS.md before selecting a topic. Record one reader question, intended audience, canonical route, and the page's distinct value. Check existing pages first; improve one if a new page would duplicate it. Prefer privacy-safe, aggregate GSC page evidence. Mark unavailable metrics UNKNOWN. Never use individual screening behavior to choose a topic.
...
- Original reviewed content revision: `83c344cd994913c3c0913e398563d1b3803909fd`. The fentanyl article object remains unchanged. The narrowly authorized September 5 overdose source/date correction above supersedes only its affected paragraph, FAQ, and source destination; focused SHA-256 tests preserve the remaining content boundary. Original review/publication dates remain historical facts, not new approval dates.
